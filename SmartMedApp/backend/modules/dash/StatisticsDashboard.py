...
class StatisticsDashboard(Dashboard):
    ...
    def _generate_before_after(self):
        df = self.pp.get_numeric_df(self.settings['data']).copy()
        df1 = self.pp.get_categorical_df(self.settings['data']).copy()
        def update_before_after(param1, param2):
            fig = go.Figure()
            li = [1, 3]
            for l in li:
                fig.add_shape(type='line',
                              ...
                              xref="x",
                              x0=l,
                              y0=min(min(df[param1]), min(df[param2]))*0.9,
                              x1=l,
                              y1=max(max(df[param1]), max(df[param2]))*1.1,
                              line=dict(color='black', width=1))

            for p1, p2, c in zip(df[param1], df[param2], df1[df1.columns[0]]):
                if p2 > p1:
                    fig.add_trace(
                        go.Scatter(x=[1, 3], y=[p1, p2], mode='lines+markers', line=dict(width=0.8, color='green'), name = c))
                else:
                    fig.add_trace(
                        go.Scatter(x=[1, 3], y=[p1, p2], mode='lines+markers', line=dict(width=0.8, color='red'), name = c))
            fig.update_yaxes(title='Значения характеристик')
            fig.update_xaxes(title='Положение ДО - Положение ПОСЛЕ', showticklabels = False)
            return fig

        self.app.callback(dash.dependencies.Output('before_after', 'figure'),
                          dash.dependencies.Input('param1', 'value'),
                          dash.dependencies.Input('param2', 'value'))(update_before_after)
        available_indicators = self.pp.get_numeric_df(self.settings['data']).columns.unique()
        return html.Div([html.Div(html.H1(children='Наклонная карта'),
                                  style={'text-align': 'center'}),
                         html.Div([
                         html.Div([
                                 html.Div([
                                     dcc.Markdown(children="Выберите показатель до:"),
                                     dcc.Dropdown(
                                         id='param1',
                                         options=[{'label': i, 'value': i}
                                                  for i in available_indicators],
                                 ...
                                 html.Div([
                                     dcc.Markdown(children="Выберите показатель после:"),
                                     dcc.Dropdown(
                                         id='param2',
                                         options=[{'label': i, 'value': i}
                                                  for i in available_indicators],
                             ...
                             dcc.Graph(id='before_after')], style={'width': '78%', 'display': 'inline-block',
                                                                   'border-color': 'rgb(220, 220, 220)',
                                                                   'border-style': 'solid', 'padding': '5px'}),
                         html.Div(dcc.Markdown(children=markdown_text_before_after),
                                  style={'width': '18%', 'float': 'right', 'display': 'inline-block'})],
                        style={'margin': '100px'}
